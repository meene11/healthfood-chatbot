"""
실험 9: 내부 QA 데이터셋으로 3개 모델 비교 평가
==================================================
실험 8과의 차이:
  - 데이터: 조원 QA(62개) → 내 documents_v2에서 생성한 QA(42개)
  - 정답: source_content 키워드 매칭 → 정확한 chunk_id + 청크 내용
  - 추가 지표: Hit@1/3/5, MRR (검색 품질)

평가 지표:
  1. 검색 품질: Hit@1 / Hit@3 / Hit@5 / MRR (chunk_id 기반)
  2. 답변 품질: LLM-as-Judge (0~3점, 정답 청크 내용 기준)
  3. 응답 시간

실행:
  python src/evaluate_exp9_internal_qa.py

세 모델을 순차 실행해 results/model_comparison/exp9_*.json/md 저장
"""
...
def save_results(summary: dict, ts: str):
    ...
    # JSON
    jp = RESULTS_DIR / f"exp9_{key}_{ts}.json"
    jp.write_text(json.dumps(summary, ensure_ascii=False, indent=2), encoding="utf-8")

    # Markdown
    mp = RESULTS_DIR / f"exp9_{key}_{ts}.md"
    lines = [
        f"# 실험 9: 내부 QA 평가 — {summary['label']}",
        f"> 평가 일시: {ts}  |  문항: {n}개  |  데이터: documents_v2 내부 생성 QA",
        "",
        "## 1. 검색 품질 (Retrieval)",
        "",
        "| 지표 | 값 |",
...
def main():
    print("=" * 55)
    print("  실험 9: 내부 QA 데이터셋 3개 모델 비교 평가")
    print("=" * 55)
